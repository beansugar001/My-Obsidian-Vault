...
自回归语言模型的目标是根据前文预测下一个 token。

### 7.2 N-gram 语言模型

传统 n-gram 模型用前 n-1 个词预测当前词：
...
神经语言模型用词向量和神经网络估计条件概率。优点：

- 能学习稠密表示。
- 能泛化到相似上下文。
- 能与更复杂结构结合，如 RNN、Transformer。
...
直观理解：模型平均每一步在多少个候选词之间困惑。越低通常越好。

注意：

- 困惑度只反映预测下一个 token 的能力。
- 不等同于事实正确性、推理能力或对齐程度。
- 不同 tokenizer 下的 perplexity 不一定可直接比较。
...
- 长距离依赖困难。
- 梯度消失：远处信息传不过来。
- 梯度爆炸：训练不稳定，需要 gradient clipping。
...
LSTM 缓解了梯度消失，但计算更复杂，仍不易并行。

### 8.4 GRU

GRU 是 LSTM 的简化版本，主要有：

- 更新门
- 重置门

GRU 参数更少，训练更快，有时效果接近 LSTM。

### 8.5 RNN 在现代 NLP 中的位置

RNN 曾经是机器翻译、文本分类、序列标注的主流模型，但后来大多被 Transformer 替代。理解 RNN 仍有价值，因为它帮助理解序列建模、隐藏状态、梯度传播和注意力机制的动机。
...
Seq2Seq 模型由 encoder 和 decoder 组成：

- Encoder 读取输入序列，生成表示。
- Decoder 根据表示逐步生成输出序列。
...
早期 Seq2Seq 把整个输入压缩成一个固定长度向量。问题是：

- 长句信息容易丢失。
- decoder 每一步都依赖同一个压缩表示。
- 对齐关系难以学习。
...
训练 decoder 时，常把真实上一个 token 输入模型，而不是模型自己生成的 token。这叫 teacher forcing。

优点：训练稳定、收敛快。

缺点：训练和推理不一致，推理时错误可能累积。

### 10.4 Beam Search

生成任务中，每一步都选概率最高的 token 可能导致局部最优。Beam search 保留多个候选序列：
